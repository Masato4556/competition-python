...
for i in range(H):
    for j in range(W):
        if S[i][j] == ".":
            ...
            roots.add(uf.root(i*W+j))

            if i + 1 < H and S[i+1][j] == "o":
                starts[uf.root(i*W+j)].add((i+1, j))
            if j + 1 < W and S[i][j+1] == "o":
                starts[uf.root(i*W+j)].add((i, j+1))
...
size = 0
for root in roots:
    cur_size = uf.size(root)
    if cur_size > size:
        size = cur_size
        start = root

roots = list(roots)
roots.sort(key=lambda x: uf.size(x), reverse=True)

max_size = roots[0]

ans = 0
for root in roots:
    if uf.size(root) == size:
        ans = max(ans, len(bfs(start, S)))
