...
init_vals = [0 if S[i] == S[i+1] else 1 for i in range(N-1)]

segtree = SegTree(init_vals, lambda a, b: a+b, 0)
...
for _ in range(Q):
    ...
    if q == 1:
        if l != 0:
            segtree.update(l-1, 1-segtree.get(l-1))
        if r != N-1:
            segtree.update(r, 1-segtree.get(r))
    else:
        print("Yes" if segtree.query(l, r) == r-l else "No")
